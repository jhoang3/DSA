...
private:
    ...
    void upheap(size_type index) {
        ...
        if(comp(c.at(index), c.at(parent_index))) { // parent !< index -> parent >= index
            return;
        } 
        // swap parent and index
        std::swap(c.at(parent_index), c.at(index)); // uses move semantics
        upheap(parent_index);
    }
    ...
    void downheap(size_type index) {
        // TODO
        // iteratively
        for(size_type element_index = index; !is_leaf(element_index);) {
            size_type max_index = left_child(element_index);
            if(right_child(element_index) < c.size())    {
                if(comp(c.at(left_child(element_index)), c.at(right_child(element_index)))) {
                    max_index = right_child(element_index);
                }
            }
            if(comp(c.at(element_index), c.at(max_index))) {
                std::swap(c.at(element_index), c.at(max_index));
