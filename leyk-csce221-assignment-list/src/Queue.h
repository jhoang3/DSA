...
class Queue {
    ...
    public:
        ...
        // I will say head is front, tail is back
        // Returns a reference to the current value at the front of the queue
...
inline bool operator==(const Queue<T, Container>& lhs, const Queue<T, Container>& rhs) { 
    // compares individual elements of queues
    if(lhs.size() != rhs.size()) {
        return false;
    }
    for (auto it = lhs.c.begin(), it2 = rhs.c.begin(); it != lhs.c.end(); ++it, ++it2) {
        if(*it != *it2) {
            return false;
        }
    }
